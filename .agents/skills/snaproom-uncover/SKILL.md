...
allowed-tools: Read Write Glob Bash(ls *) Bash(node .Codex/scripts/project/project-state.mjs *)
---
...
Follow the generic file convention in `.Codex/rules/project.md`. Use `ls -a` to inspect project/source/output directories before reading JSON details.
...
```bash
node .Codex/scripts/project/project-state.mjs --world "$0" --stage-input
```
...
3. Read `SNAPROOM.md` in this skill directory and follow its JSON contract exactly.
...
11. After object files are written, handle the clean plate decision:
   - In one-shot mode, continue with `Agent(snaproom-plate)` and wait for it before world generation.
