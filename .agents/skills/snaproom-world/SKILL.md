...
allowed-tools: Read Write Glob Bash(ls *) Bash(node .Codex/scripts/project/project-state.mjs *) Bash(node .Codex/scripts/project/ensure-local-assets.mjs *) Bash(node .Codex/scripts/world/generate-world.mjs *)
context: fork
agent: snaproom-world
---
...
```bash
node .Codex/scripts/world/generate-world.mjs --world "$0" --prompt "<empty-environment world caption>"
```
...
```bash
node .Codex/scripts/project/ensure-local-assets.mjs --from "worlds/$0/output/world/<N>-world.json"
```
